...
def remove_doc_from_shelf(doc_number):
    for directory_number, directory_docs_list in directories.items():
        if doc_number in directory_docs_list:
            ...
            break
    return doc_number  #добавил возвращение удаляемого документа
...
def append_doc_to_shelf(doc_number, shelf_number):
    ...
    return directories[shelf_number] #добавил return
...
def move_doc_to_shelf(doc, shelf): #вместо input добавил аргументы
    user_doc_number = doc #input('Введите номер документа - ')
    user_shelf_number = shelf #input('Введите номер полки для перемещения - ')
    ...
    # добавил return
    return user_doc_number, user_shelf_number #добавил return
...
def show_document_info(document):
    ...
    return doc_type, doc_number, doc_owner_name #добавил return
